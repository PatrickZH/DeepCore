...
class DeepFool(EarlyTrain):
    ...
    def finish_run(self):
        ...
        for i, (inputs, targets) in enumerate(batch_loader):
            ...
            r[(i * self.args.selection_batch):(i * self.args.selection_batch + targets.shape[0])] = self.deep_fool(inputs)
    ...
    def deep_fool(self, inputs):
        ...
        for _ in range(self.max_iter):
            ...
            w_k = []
            for c in range(self.args.num_classes):
                w_k.append(torch.autograd.grad(f_all[:, c].sum(), cur_inputs, retain_graph=False if c + 1 == self.args.num_classes else True)[0].flatten(1))
            w_k = torch.stack(w_k, dim=0)
            w_k = w_k - w_k[first_preds, boolean_mask[boolean_mask]].unsqueeze(0)
            w_k_norm = w_k.norm(dim=2)

            w_k_norm[first_preds, boolean_mask[boolean_mask]] = 1. # Set w_k_norm for preds positions to 1. to avoid division by zero.
