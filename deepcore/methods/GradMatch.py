...
class GradMatch(EarlyTrain):
    ...
    def calc_gradient(self, index=None, val=False):
        ...
        gradients = torch.zeros([sample_num, self.args.num_classes * (self.embedding_dim + 1)], requires_grad=False, device=self.args.device)
        ...
        for i, (input, targets) in enumerate(batch_loader):
            ...
            with torch.no_grad():
                ...
                weight_parameters_grads = self.model.embedding_recorder.embedding.cpu().view(batch_num, 1, self.embedding_dim).repeat(1,self.args.num_classes,1) * bias_parameters_grads.view(
            ...
            if self.balance:
                ...
                for c in range(self.args.num_classes):
                    ...
                    if self.args.device == "cpu":
                        ...
                        cur_weights = self.orthogonal_matching_pursuit_np(cur_gradients.numpy().T, cur_val_gradients.numpy(), budget=round(len(class_index) * self.fraction))
                    else:
                        cur_weights = self.orthogonal_matching_pursuit(cur_gradients.to(self.args.device).T, cur_val_gradients.to(self.args.device), budget=round(len(class_index) * self.fraction))
                ...
                if self.args.device == "cpu":
                    # Compute OMP on numpy
                    cur_weights = self.orthogonal_matching_pursuit_np(cur_gradients.numpy().T, cur_val_gradients.numpy(),  budget=self.coreset_size)
                else:
                    cur_weights = self.orthogonal_matching_pursuit(cur_gradients.T, cur_val_gradients, budget=self.coreset_size)
...
"""
    ...
    def OrthogonalMP_REG_torch(self, A, b, tol=1E-4, nnz=None, positive=True, lam=0, device="cuda"):
        ...
        with torch.no_grad():
            ...
            normb = b.norm().item()
            indices = torch.ones(n, dtype=bool, device="cuda")
            all_idx = torch.arange(n, device='cuda')
            ...
            for i in range(nnz):
                ...
                if positive:
                    index = torch.argmax(projections[indices])
                else:
                    index = torch.argmax(torch.abs(projections[indices]))

                #if index in indices:
                    #break

                #indices.append(index)
                index = all_idx[indices][index]
                indices[index] = False

                if indices.sum().item() == 1:
                    A_i = A[:, index]
                    x_i = projections[index] / torch.dot(A_i, A_i).view(-1)
                    A_i = A[:, index].view(1, -1)
                else:
