...
def main():
    ...
    if args.save_path != "":
        checkpoint_name = "{dst}_{net}_{mtd}_exp{exp}_epoch{epc}_{dat}_{fr}_".format(dst=args.dataset, net=args.model,
                                                                                mtd=args.selection,
                                                                                dat=datetime.now(), exp=start_exp,
                                                                                epc=args.epochs,fr=args.fraction)
    for exp in range(start_exp, args.num_exp):
        ...
        if "subset" in checkpoint.keys():
            ...
            selection_args = dict(epochs=args.selection_epochs, selection_method='Entropy',
                                  balance=args.balance, #network=network,optimizer=optimizer, criterion=criterion,
                                  greedy="LazyGreedy",function="GraphCut",
        ...
        if args.save_path != "":
            ...
            checkpoint_name = "{dst}_{net}_{mtd}_exp{exp}_epoch{epc}_{dat}_{fr}_".format(dst=args.dataset, net=args.model, mtd=args.selection,
                            dat=datetime.now(), exp=exp + 1, epc=args.epochs,fr=args.fraction)
        checkpoint = {}
